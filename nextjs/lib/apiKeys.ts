...
export async function resolveApiKey(): Promise<Session | null> {
  return resolveApiKeyFromToken((await headers()).get("x-api-key"));
}

// Token-based core. Lets non-X-API-Key callers reuse the same SHA-256 lookup path —
// the MCP server / getAuthorizedUser reading `Authorization: Bearer grm_...` pass the
// raw token here instead of relying on the request's x-api-key header.
export async function resolveApiKeyFromToken(token: string | null): Promise<Session | null> {
