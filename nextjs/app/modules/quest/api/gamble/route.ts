...
// POST /modules/quest/api/gamble — spend coins to roll a d20 and recover HP (short rest).
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!result.ok && result.reason === 'full_health') {
      return NextResponse.json({ error: 'Already at full health', balance: result.balance }, { status: 400 });
    }
