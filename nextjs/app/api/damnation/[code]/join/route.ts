...
import { joinSession } from "@/app/modules/damnation/lib/mutationFunctions";
...
// POST /api/damnation/[code]/join — join as a new player while joining is open.
...
export async function POST(request: Request, { params }: { params: Promise<{ code: string }> }) {
  try {
    ...
    const joined = await joinSession(session.id, body.op_id, body.display_name, body.color_key);
